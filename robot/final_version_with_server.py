...
def handle_request(request):
    ...
    dest_x_milimeter = request[0]#int(input('X in milimeter>> '))
    dest_y_milimeter = request[1]#int(input('Y in milimeter>> '))
...
if __name__ == "__main__":
    ...
    t_reportpos = threading.Thread(target=ser.report_position, args=(narslam,))
    t_reportpos.start()
    print('position reporting started')
...
#            exit(0)
        cmd = input("please give me order\n(\"run\": run main scenario | 0,1,2,3,4: move)\n>> ")
        if cmd == 'run':
            ser.report_progress()
            while True:
                if ntdriver.server.request is not None:
                    handle_request(ser.request)
                print(narslam.x, narslam.y, narslam.theta)
                time.sleep(1)
